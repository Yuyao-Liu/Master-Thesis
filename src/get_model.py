...
def heron_approximationdd():
    ...
    translation = np.array([0.5,-0.3,0.5])
    theta = np.radians(45)
    rotation = np.array([[np.cos(theta), -np.sin(theta), 0], [-np.sin(theta), -np.cos(theta), 0], [0, 0, 1]])
    base_placement = pin.SE3(rotation, translation)
    
    
    
    # 1 body frame
    # base_body_frame_id = model_mobile_base.addBodyFrame("base_body", parent_id, base_placement, -1)
    # 2 joint
    ...
    MOBILE_BASE_REVOLUTE_ID = model_mobile_base.addJoint(
        parent_id, pin.JointModelHZ(0), joint_placement.copy(), revolute_joint_name
    )
    # model_mobile_base.addJointFrame(
    #     MOBILE_BASE_REVOLUTE_ID, base_body_frame_id
    # )
    # model_mobile_base.addJointFrame(MOBILE_BASE_REVOLUTE_ID, base_frame_id)
    MOBILE_BASE_PRISMATIC_ID = model_mobile_base.addJoint(
        MOBILE_BASE_REVOLUTE_ID,
        pin.JointModelPX(),
        joint_placement.copy(),
    ...
    # there are no position limit by default and that is what we want.
    # TODO: put in heron's values
    num_joints = len(model_mobile_base.joints)
    print(f"Total joints in model: {num_joints}")  # 期望输出：3（包括 root）

    # **初始化速度限制**
    model_mobile_base.velocityLimit[0] = 2  # X 方向最大速度
    model_mobile_base.velocityLimit[1] = 2  # 角速度最大值

    # **初始化力矩限制**
    # model_mobile_base.effortLimit = np.zeros(num_joints)
    model_mobile_base.effortLimit[0] = 200
    model_mobile_base.effortLimit[1] = 200
    ...
    # have to add the frame manually
    base_frame = model_mobile_base.addFrame(
        pin.Frame(
            "base",
            MOBILE_BASE_PRISMATIC_ID,
            0,
    ...
    for frame in model_mobile_base.frames:
        print(frame.name, frame.type)
    # frame-index should be 1
    model, visual_model = pin.appendModel(
        model_mobile_base,
        model_arm,
...
def heron_approximationee():
    ...
    # because mobile yumi's base is a planar joint
    MOBILE_BASE_JOINT_ID = model_mobile_base.addJoint(
        parent_id, crocoddyl.ActionModelUnicycle(), joint_placement.copy(), joint_name
    )
    # we should immediately set velocity limits.
    # there are no position limit by default and that is what we want.
    # TODO: put in heron's values
