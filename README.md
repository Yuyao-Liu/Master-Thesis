...
# Running Control Code on Real Robot and in Simulation

---

## Prerequisites

- All commands and code execution must be performed **inside the ROS container** provided by [robotlab](https://git.cs.lth.se/robotlab/ros-containers/-/tree/dev?ref_type=heads).  
  &gt; ⚠️ Make sure you have the correct version of ROS container running before proceeding.  
  &gt; Marko is the go-to expert for setup and troubleshooting with this container.


---

## Step 1: Verify in Simulation (Recommended)

Before running on the physical robot, test your control code in the **Gazebo simulator** to avoid uncontrolled base movements.

### 1. Launch Gazebo Simulation
Open the **first terminal** (inside the ROS container) and run (not provided in this repository; obtain it through the ROS container.):
...
- Reference: [mir_gazebo repository](https://git.cs.lth.se/robotlab/heron/mir-robot/-/tree/ros2?ref_type=heads)
- Wait until the Gazebo window opens and the robot’s mobile base appears.

### 2. Run Control Code


Open the **second terminal** (inside the ROS container), go to the directory where your code is located and run your main control script in simulation mode:


```bash
python mainloop_real.py

```

- Ensure `real = False` in the code.
- Observe the robot’s motion in both **Gazebo** and **Meshcat** visualizations.
- Confirm that the base and arm behave as expected, without unsafe motions.


## Step 2: Deploy on Real Robot


Once the simulation test is successful:


1. **Stop the Gazebo simulation** in the first terminal to free resources for the real robot.
2. Power on and network the real Heron robot.

### 1. Launch Robot Bringup


In the **first terminal**, launch the real robot’s bringup (not provided in this repository; obtain it through the ROS container.):


```bash
cd ~/colcon_ws/src/robots/heron_robot/heron_robot/launch
ros2 launch heron_bringup.launch.py

```

- Wait for **RViz** to display the robot’s map and localization data.
- This step is mandatory to subscribe to the robot’s ROS2 topics and send commands.

### 2. Run Control Code on Real Hardware


In the **second terminal**, run your main control script in real mode:
